...
// ViewDataBind 封装使用
public abstract class BaseBindingAdapter<M, DB extends ViewDataBinding> extends Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        DB binding = DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()), this.getLayoutResId(viewType), parent, false);
        return new BaseBindingViewHolder<DB>(binding);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position){
        DB binding = DataBindingUtil.getBinding(holder.itemView);
        LogUtils.d(" onBindViewHolder " + items);
